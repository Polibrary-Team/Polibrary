...
using Polytopia.Data; //i stuck a comment in here, what are you going to do about it? cry? GOOD. CRY MORE
using Steamworks.Data;
using Une = UnityEngine; //haha man i love this engine. its soo trustworthy and never tried to bankrupt thousands of indie devs because of corporate greed hahaha
...
public static class Main //KRIS!! YOU BACK FOR THE [Round Trip Around Spicetown]?? HERE FOR THE [Comments]?? THE [Hyperlink Blocked]??
{
    private static ManualLogSource? LogMan1997;

    public static void Load(ManualLogSource logger)
    {
        logger.LogMessage("Holy cungareeedooo do I feel GOOD"); //imagine getting banned for publishing without verification
        ...
        Harmony.CreateAndPatchAll(typeof(Main)); //I AM NOT READING HARMONY DOCS. I REPEAT: I AM NOT READING HARMONY DOCS
        LogMan1997 = logger;
        LogMan1997!.LogInfo("HEY      EVERY      !!! IT'S ME, EVERY [[Goober]]'s FAVORITE [ModLogger1997]!!"); //who the fuck let this guy in?
        LogMan1997!.LogInfo("SPAMT-"); //no seriously
        LogMan1997!.LogInfo("LOGMAN     G.          LOGMAN"); //WHO THE FUCK LET THIS GUY IN??
        LogMan1997!.LogInfo("KRIS LOOK! I AM A %&{@ING LOGGER!!");
    }
    ...
    public static bool ImprovementDataExtentions_GetCityRewardsForLevel(ref Il2CppStructArray<CityReward> __result, ImprovementData data, int level)
    {
        ...
        foreach (CityReward reward1 in list)
        {
            ...
            {
                num2 = cityRewardData.order + 1;
                while (orderArray.Contains(num2))
                {
                    num2++;
                }
            }
        ...
        for (int i = 0; i < gameState.Map.Tiles.Length; i++)
        {
            ...
            if (explored)
            {
                ...
                if (flag)
                {
                    ...
                    bool flag3 = tileData.terrain == TerrainData.Type.Field || tileData.terrain == TerrainData.Type.Forest;
                    if (flag3)
                    {
                        num++;
                    }
