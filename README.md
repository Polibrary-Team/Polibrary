...
A library mod that helps modders make mods better & faster.

Latest version: 2.1.4

# Installation (User or GLD Modding)
These instructions are for users or GLD modders. For polyscript usage, scroll a bit down.

## Verified (Recommended)
This is the safest way to download Polibrary. The mod is checked by Polymod developers for any malware, so it's safe to download.

Download the **verified** .polymod file from [the polymod website](https://polymod.dev).
...
### Github versions
We provide versions on github aswell, if you find it more convenient this way.

### "DIY" versions
These are literally just source codes. Extremely unstable, contain a lot of unfinished code, but the most up-to-date. From the repository, download the `Polibrary.polymod` file.

# Installation (for Polyscript)
Follow the same steps as in the regular method, but we also need 1 extra thing: the `Polibrary.dll` file. 

Simply convert the `Polibrary.polymod` file to a .zip file, extract it, and take the `Polymod.dll` file. Then you can place it in your project. You can also download the .dll from our UNVERIFIED github releases directly.

For more in-depth guidance, you can take a peek at [the template project that Polibrary is built upon](https://github.com/wasdeilonn/wasdTemplate).
# Usage
